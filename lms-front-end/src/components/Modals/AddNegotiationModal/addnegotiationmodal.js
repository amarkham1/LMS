...
function validateTheSpace(propertyname, unit, gla, status) {
  return {
...
class AddNegotiationModal extends React.Component {
  ...
  async componentDidMount() {
    await fetch(
      "https://intense-temple-63357.herokuapp.com/adddealmodal/property",
      {
        method: "get",
  ...
  onFreeRentMonthsChange = event => {
    ...
    let gfrentend = gfrentstartMoment
  ...
  handleBlur = field => event => {
    ...
    if (field === "cdate" || field === "term") {
      if (this.state.cdate && this.state.term) {
        ...
        let expirydate = cdateMoment
          .add(this.state.term, "months")
          .subtract(1, "days")
          .format("YYYY-MM-DD");
  ...
  canNext(page) {
    if (page === "thespace") {
      ...
      const errors = validateFinancialTerms(
        this.state.rent1,
        this.state.rent1months,
        this.state.ti,
        this.state.intcomm,
        this.state.extcomm,
        this.state.llw,
        this.state.gfrent,
        this.state.gfrentmonths
      );
      const isDisabled = Object.keys(errors).some(x => errors[x]);
      return !isDisabled;
    }
  ...
  render() {
    const errorsTheSpace = validateTheSpace(
      this.state.propertyname,
      this.state.unit,
      this.state.gla,
      this.state.status
    );
    const errorsTiming = validateTiming(
      this.state.cdate,
      this.state.term,
      this.state.fdate
    );
    const errorsFinancialTerms = validateFinancialTerms(
      this.state.rent1,
      this.state.rent1months,
      this.state.ti,
      this.state.intcomm,
    ...
    const isTheSpaceDisabled = Object.keys(errorsTheSpace).some(
      x => errorsTheSpace[x]
    );
    const isTimingDisabled = Object.keys(errorsTiming).some(
      x => errorsTiming[x]
    );
    const isFinancialTermsDisabled = Object.keys(errorsFinancialTerms).some(
      x => errorsFinancialTerms[x]
    );
    ...
    const showHideClassName = this.props.show
      ? "neg-modal display-block"
      : "neg-modal display-none";
    return (
      <div className={showHideClassName}>
        <React.Fragment>
          {this.state.loaded && (
            <div className="neg-modal-main">
              <div className="neg-modal-title">
                Add New Stage of Negotiation
              </div>
              <div className="neg-modal-body">
                <div className="neg-modal-body-header">
                  ...
                  <p
                    className={
                      this.state.modalcategory["financialterms"]
                        ? "neg-header-sub active-sub"
                        : "neg-header-sub"
                    }
                  >
                  ...
                  <React.Fragment>
                    <div className="neg-boxcontainer">
                      <div className="leftbox">
                        <p
                          className={
                            shouldMarkTheSpaceError("propertyname")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                      ...
                      <div className="rightbox">
                        ...
                        <p
                          className={
                            shouldMarkTheSpaceError("status")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                  ...
                  <React.Fragment>
                    <div className="neg-boxcontainer">
                      <div className="leftbox">
                        ...
                        <p
                          className={
                            shouldMarkTimingError("fdate")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                          Fixturing Start Date: (YYYY-MM-DD)
                        </p>
                        <input
                          type="input"
                  ...
                  <React.Fragment>
                    <div className="neg-boxcontainer">
                      <div className="leftbox">
                        <p
                          className={
                            shouldMarkFinancialTermsError("rent")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                        ...
                        <p
                          className={
                            shouldMarkFinancialTermsError("ti")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                        ...
                        <p
                          className={
                            shouldMarkFinancialTermsError("llw")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                      ...
                      <div className="rightbox">
                        <p
                          className={
                            shouldMarkFinancialTermsError("extcomm")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                        ...
                        <p
                          className={
                            shouldMarkFinancialTermsError("intcomm")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
                        ...
                        <p
                          className={
                            shouldMarkFinancialTermsError("gfrent")
                              ? "input-title-error"
                              : "input-title"
                          }
                        >
