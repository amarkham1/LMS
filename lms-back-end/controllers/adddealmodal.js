const handleAddDealModalTenant = (req, res, db) => {
  db.select('tenantname').from('tenants')
    .then(tenants => {
        res.json(tenants)
    })
...
const handleAddDealModalProperty = (req, res, db) => {
  db.select('propertyname').from('properties')
    .then(properties => {
        res.json(properties)
    })
    .catch(err => res.status(400).json('error getting deals'))
...
const handleAddDealModalLLBroker = (req, res, db) => {
  db.select('brokername').from('llbroker')
    .then(brokers => {
        res.json(brokers)
    })
    .catch(err => res.status(400).json('error getting deals'))
...
const handleAddDealModalTTBroker = (req, res, db) => {
  db.select('brokername').from('ttbroker')
    .then(brokername => {
        res.json(brokername)
    })
    .catch(err => res.status(400).json('error getting deals'))
